...
acc = Account ('123456789012', 3000, 1234)
...
class Product:
   ...
   def final_price(self):
      return self.__price
my_product = Product('Картофель', 25)
...
my_course.add_student('Алиса')
my_course.add_student('Василиса')
my_course.add_student('Вероника')
print(my_course.get_students())
my_course.remove_student('Василиса')
print(my_course.get_students())
...
order = Order('Лагман', 250)
dine_in_order = DineInOrder('Лагман', 250)
print(dine_in_order.calculate_total())
take_away_order = TakeAwayOrder('Лагман', 250)
print(take_away_order.calculate_total())
delivery_order = DeliveryOrder('Лагман', 250)
...
class Warrior(Character):
    def attack(self): print(f"{self.name} атакует мечом.")
class Mage(Character):
    def attack(self): print(f"{self.name} атакует магией.")
class Archer(Character):
    def attack(self): print(f"{self.name} атакует из лука.")
warrior = Warrior('Ёнэ',2400,90)
mage = Mage('Аврора',2400,100)
archer = Archer('Афелий',2300,90)
warrior.attack()
mage.attack()
archer.attack()
print(warrior)
...
a = AudioFile("Песня Егора Крида", 3)
b = VideoFile("Клип Егора Крида", 5)
c = Podcast("Интервью Егора Крида", 40)
a.play()
b.play()
c.play()
...
class Lion(Animal):
    ...
    def sleep(self): print('Лев спит вытянувшись на животе.')
class Elephant(Animal):
    def eat(self): print('Слон питается травой.')
    def sleep(self): print('Слон спит стоя, используя опоры.')
class Snake(Animal):
    def eat(self): print('Змея питается маленькими животными.')
    def sleep(self): print('Змея спит свернувшись клубком.')
lion = Lion()
elephant = Elephant()
snake = Snake()
lion.eat(); lion.sleep()
